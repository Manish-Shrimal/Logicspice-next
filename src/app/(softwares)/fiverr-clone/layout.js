...
export async function generateMetadata({ params, searchParams }) {
  try {
    ...
    const res = await fetch(`${MetadataApi}/fiverr-clone`, { cache: "no-store" });
    const product = await res.json();

    let text = product?.data?.schema || "";
    let schemaOrg = null;

    // Process JSON schema (clean up escape characters)
    if (text) {
      try {
        const cleanedText = text
          .replace(/\\r\\n|\\n|\\r/g, "") // Remove \r\n, \n, \r
      ...
      "mainEntity": [
        {
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "User can search for gigs by selecting categories and subcategories or entering relevant keywords.",
          },
        },
        {
          "@type": "Question",
          "name": "Can a user view the profile of a gig owner?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes, users can view the profile of the gig owner, including total reviews and other gigs.",
          },
        },
        {
          "@type": "Question",
          "name": "How can users manage their gigs?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Users can add, edit, and delete their gigs, including pictures and deal details.",
          },
        },
        {
          "@type": "Question",
          "name": "How long does customization take?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Customization typically takes 5-6 weeks, depending on complexity and data migration.",
          },
        },
        {
          "@type": "Question",
          "name": "Can a user rate a gig without purchasing it?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "No, users can only rate and review gigs after purchase.",
          },
        },
      ],
    };
    ...
    return {
      ...
      description: product?.data?.meta_description || "Fiverr Clone Script for freelancers",
      keywords: product?.data?.meta_keyword || "Fiverr, clone, freelancers",
      alternates: { canonical: `${Domain}/fiverr-clone` },
      robots: {
        index: true,
        follow: true,
        googleBot: {
      ...
      openGraph: {
        title: product?.data?.meta_title || "Fiverr Clone",
        description: product?.data?.meta_description || "Freelancer marketplace script",
        url: `${Domain}/fiverr-clone`,
        type: "website",
        image: `${Domain}/images/og-image.jpg`, // Update with your actual OG image
      },
      schemaOrg,
      faqSchema,
    };
  ...
  return (
    <html lang="en">
      ...
      {metadata.faqSchema && (
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(metadata.faqSchema) }}
        />
      )}
    </html>
  );
